...
features:
  - icon: 🔑
    ...
    details: Agentes recebem chaves efêmeras com limite de tempo e valor. Eles nunca têm a chave privada da carteira. As sessões expiram automaticamente.

  - icon: 💰
    title: Limites de Gasto Diários
    details: Defina o quanto cada agente pode gastar por dia e por transação. O programa on-chain aplica — sem exceções.

  - icon: 🛡️
    title: Lista de Programas Permitidos
    details: Seu bot LP fica na Meteora. Seu sniper fica na Raydium. Nenhum agente pode chamar contratos não autorizados, mesmo se comprometido.

  - icon: 🚨
    title: Botão de Emergência
    details: Uma chamada congela todos os agentes instantaneamente. Você sempre tem controle total.

  - icon: 👥
    title: Recuperação com Guardiões
    details: Perdeu sua chave? Os guardiões podem rotacionar a autoridade da carteira via multi-assinatura. Seus fundos sempre recuperáveis.

  - icon: 🤖
    title: Suporte Multi-Agente
