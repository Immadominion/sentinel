//! Utility functions for the Seal Wallet program.
//!
...
pub fn verify_account_owner_and_discriminator(
    ...
    if data.len() < 8 {
        return Err(ProgramError::from(SealError::InvalidDataLength));
    }
    if data[..8] != *expected_discriminator {
        return Err(ProgramError::from(SealError::InvalidAccountData));
    }
    Ok(())
}
...
pub fn load_account<T: BorshDeserialize>(
    ...
    T::try_from_slice(&data[..size])
        .map_err(|_| ProgramError::from(SealError::DeserializationError))
}
...
pub fn save_account<T: BorshSerialize>(
    ...
    let serialized =
        borsh::to_vec(value).map_err(|_| ProgramError::from(SealError::SerializationError))?;
