/**
 * Constants for the Sentinel smart wallet program.
 */

import { PublicKey } from "@solana/web3.js";

// Program ID — deployed to devnet on Feb 25, 2026
export const SENTINEL_PROGRAM_ID = new PublicKey(
    "EV3TKRVz7pTHpAqBTjP8jmwuvoRBRCpjmVSPHhcMnXqb"
);

// PDA Seeds
export const WALLET_SEED = Buffer.from("sentinel");
export const AGENT_SEED = Buffer.from("agent");
export const SESSION_SEED = Buffer.from("session");

// Account Discriminators (first 8 bytes)
export const SMART_WALLET_DISCRIMINATOR = Buffer.from("SentWalt"); // 8 bytes
export const AGENT_CONFIG_DISCRIMINATOR = Buffer.from("SentAgnt");
export const SESSION_KEY_DISCRIMINATOR = Buffer.from("SentSess");

// Instruction Discriminators (first byte)
export enum InstructionDiscriminant {
    CreateWallet = 0,
    RegisterAgent = 1,
    CreateSessionKey = 2,
    ExecuteViaSession = 3,
    RevokeSession = 4,
    UpdateSpendingLimit = 5,
    AddGuardian = 6,
    RecoverWallet = 7,
    DeregisterAgent = 8,
    CloseWallet = 9,
}

// Limits
export const MAX_GUARDIANS = 5;
export const MAX_ALLOWED_PROGRAMS = 8;
export const MAX_ALLOWED_INSTRUCTIONS = 16;
export const MAX_AGENT_NAME_LENGTH = 32;

// Default values
export const DEFAULT_SESSION_DURATION_SECS = BigInt(24 * 60 * 60); // 24 hours
export const MAX_SESSION_DURATION_SECS = BigInt(7 * 24 * 60 * 60); // 7 days

// Account sizes (MUST match Rust - verified from programs/sentinel-wallet/src/state/*.rs)
export const SMART_WALLET_SIZE = 245; // 8+32+1+8+1+1+160+8+8+8+8+1+1
export const AGENT_CONFIG_SIZE = 540; // 8+32+32+32+1+1+1+256+1+128+8+8+8+8+8+8
export const SESSION_KEY_SIZE = 154;  // 8+32+32+32+1+8+8+8+8+8+1+8
